...
export function convertMapEntriesToObject<V>(
  mapEntries: Array<[string, V]>,
): Record<string, V> {
  const obj: Record<string, V> = {}
...
export function date(date: dayjs.ConfigType) {
  ...
  return (
    <span>
      ...
      <br />
      <span className="formatted">{dDate.format('MMM D, YYYY h:mma')}</span>
    </span>
  )
}
