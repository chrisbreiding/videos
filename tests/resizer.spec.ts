...
describe('Resizer', () => {
  ...
  test('clamps to minHeight when dragged above the top of the allowed range', async ({ page }) => {
    await setup(page, { viewportHeight: 800 })
  ...
  test('clamps to maxHeight when dragged below the bottom of the allowed range', async ({ page }) => {
    await setup(page, { viewportHeight: 800 })
  ...
  test('ignores mouse movement and mouseup when not dragging', async ({ page }) => {
    await setup(page, { viewportHeight: 800 })
  ...
  test('removes its document listeners when unmounted', async ({ page }) => {
    ...
    const resizerBox = await page.evaluate(() => ({ x: window.innerWidth / 2, y: 400 }))
