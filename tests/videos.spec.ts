...
describe('Viewing a Channel and Its Videos', () => {
  test('displays channel videos when clicking on a channel', async ({
    page,
  }) => {
    await setupApp(page, {
      subs: {
...
describe('Searching Within a Channel', () => {
  test('submits the channel search and updates the URL', async ({ page }) => {
    await setupApp(page, {
      ...
      videos: [createVideo({ id: 'video-1', title: 'iPhone Review' })],
    })
    ...
    const searchInput = page.locator(
      '.search input[placeholder="Search Channel"]',
    )
  ...
  test('submits the search when pressing Enter in the input', async ({
    page,
  }) => {
    await setupApp(page, {
      subs: {
  ...
  test('shows only the videos matching the channel search query', async ({
    page,
  }) => {
    await setupApp(page, {
      subs: {
  ...
  test('shows only the videos matching a playlist search query', async ({
    page,
  }) => {
    await setupApp(page, {
      subs: {
...
describe('Adding Videos to a Custom Playlist from a Channel', () => {
  ...
  test('can remove a video from a custom playlist', async ({ page }) => {
    await setupApp(page, {
      ...
      videos: [createVideo({ id: 'video-1', title: 'Already Added Video' })],
    })

    await page.goto('/subs/channel-1')

    await expect(page.getByText('Already Added Video')).toBeVisible({
      timeout: 10000,
    })
...
describe('Viewing a Custom Playlist Without Videos', () => {
  test('shows empty message when custom playlist has no videos', async ({
    page,
  }) => {
    await setupApp(page, {
      subs: {
...
describe('Viewing a Custom Playlist With Videos', () => {
  test('displays videos in custom playlist', async ({ page }) => {
    ...
    await expect(page.getByText('Favorite Video 1')).toBeVisible({
      timeout: 10000,
    })
    await expect(page.getByText('Favorite Video 2')).toBeVisible()
  })

  test('custom playlist videos can be reordered (sortable)', async ({
    page,
  }) => {
    await setupApp(page, {
      subs: {
    ...
    await expect(page.getByText('Draggable Video')).toBeVisible({
      timeout: 10000,
    })
  ...
  test('dragging a video by its handle reorders the custom playlist', async ({
    page,
  }) => {
    await setupApp(page, {
      subs: {
  ...
  test('dragging videos after navigating from the All Subs page reorders correctly', async ({
    ...
    for (let i = 1; i <= steps; i++) {
      const y =
        firstHandleBox.y +
        ((thirdItemBox.y + thirdItemBox.height / 2 - firstHandleBox.y) * i) /
          steps
      await page.mouse.move(firstHandleBox.x + firstHandleBox.width / 2, y)
    }
...
describe('Viewing a Playlist and Its Videos', () => {
  test('displays playlist videos', async ({ page }) => {
    ...
    await expect(page.getByText('Playlist Video 1')).toBeVisible({
      timeout: 10000,
    })
...
describe('Paginating Through a Channel', () => {
  test('can navigate to older videos', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Channel Video 1', exact: true }),
  ...
  test('can navigate backwards and forwards', async ({ page }) => {
    ...
    const page1Videos = Array.from({ length: 25 }, (_, i) =>
      createVideo({
        id: `video-p1-${i + 1}`,
        title: `First Page Vid ${i + 1}`,
      }),
    )
    const page2Videos = Array.from({ length: 25 }, (_, i) =>
      createVideo({
        id: `video-p2-${i + 1}`,
        title: `Second Page Vid ${i + 1}`,
      }),
    )
    ...
    await expect(
      page.getByRole('heading', { name: 'First Page Vid 1', exact: true }),
    ).toBeVisible({ timeout: 10000 })
...
describe('Paginating Through a Playlist', () => {
  test('playlist pagination works', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Playlist Vid 1', exact: true }),
    ).toBeVisible({ timeout: 10000 })
